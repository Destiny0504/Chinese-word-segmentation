...
class split_dataset():
  ...
  def split_data(self):
    return train_test_split(self.data, test_size=0.1, random_state=42, shuffle=False)
...
if __name__ == "__main__":
  ...
  for k, v in a.bigram_calculation().items():
    if v[0] > 20 or v[1] > 20:
      if b.bigram.get(k) is not None and (
        torch.Tensor(b.bigram.get(k)).softmax(dim=-1)[0] - torch.Tensor(a.bigram.get(k)).softmax(dim=-1)[0]
      ) > 0.2:
