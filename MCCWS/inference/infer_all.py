...
def get_args():
    ...
    parser.add_argument(
        "--checkpoint_step", default=0, help="each step between checkpoint", type=int
    )
    parser.add_argument(
        "--testset_token", action="append", default=[], help="Select the dataset"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--criteria",
        default=10,
        help="How many criteria do you want to infer?",
        type=int,
    )
    parser.add_argument(
        "--model_name",
...
def main(args):
    ...
    for ckpt in range(
        args.start_checkpoint, args.end_checkpoint + 2, args.checkpoint_step
    ):
                    ...
                    for i in range(len(segmentation)):
                        ...
                        for j in range(len(text[i])):
                            ...
                            if segmentation[i][j - space_counter] < 2:
                                if (
                                    segmentation[i][j - space_counter] == 0
                                    and len(ans) > 0
                                ):
                                    tmp.append(
                                        ans
