...
def preprocess(data, tokenizer: transformers.PreTrainedTokenizerFast, replace_rate: float):
  ...
  tokenized_str.input_ids[:, 1] = tokenized_str.input_ids[:, 1].masked_fill_(mask, 21132)
...
def get_args():
  ...
  parser.add_argument('--weight_decay', default=1e-2, help="This is the value of optimizer AdamW's weight decay", type=float)
  parser.add_argument('--smooth_factor', default=0.1, help='This is the label smoothing factor for loss function', type=float)
  parser.add_argument('--gpu', default=0, help="Select the model for training.", type=int)
  parser.add_argument('--model_name', default=None, help="Select the model from hugging face.", type=str)
  parser.add_argument('--exp_name', default='cws_model', help="Name your experiment.", type=str)
  parser.add_argument('--trainset_token', action='append', default=[], help="Select the dataset")
  parser.add_argument('--start_log', default=50000, help="The step that start logging", type=int)
  parser.add_argument('--accumulation_step', default=2, help="Gradient accumulation step", type=int)
...
def main(args):
  ...
  train_loader = DataLoader(
    ...
    shuffle=True,
    collate_fn=lambda x: preprocess(data=x, tokenizer=tokenizer, replace_rate=args.replace_rate),
  )
  ...
  segmentation_loss_fn = nn.CrossEntropyLoss(ignore_index=4, label_smoothing=args.smooth_factor)
  ...
  scheduler = MCCWS.util.load_scheduler(
    optimizer=optimizer,
    num_warmup_steps=(len(dataset) * EPOCH // args.accumulation_step // args.batch_size // 10) + 1,
    num_training_steps=(len(dataset) * EPOCH // args.accumulation_step // args.batch_size) + 1
  )
  ...
  for epoch in range(EPOCH):
    ...
    for data, target in train_data:
      ...
      criterion_loss = criterion_loss_fn(criteria.view(-1, 10), criteria_target.view(-1))
      
      # tensor.view() needs the tensor be contagious. So we use reshape right here.
      # Calculating the loss of word segmentation.
      segmentation_loss = segmentation_loss_fn(segmentation.view(-1, 4), target.reshape(target.size()[0] * target.size()[1]))
      ...
      if count % LOG_STEP == 0:
        segmentation = torch.masked_select(segmentation.argmax(dim=-1), (target < 4))
        ...
        train_data.set_description(f'Epoch :{epoch}' + f' loss :{round(loss.item(), 4)}')
