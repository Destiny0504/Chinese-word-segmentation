...
class CWSmodel(nn.Module):
    ...
    def forward(self, batch_x):
        ...
        model_predict.last_hidden_state = self.dropout(
            model_predict.last_hidden_state[:, 1:, :]
        )

        # Word segmentation
        model_predict.last_hidden_state = self.segmentor(
            model_predict.last_hidden_state[:, 1:, :]
        )
    ...
    def inference(self, batch_x):
        ...
        model_predict.last_hidden_state = self.segmentor(
            model_predict.last_hidden_state[:, 2:, :]
        )
